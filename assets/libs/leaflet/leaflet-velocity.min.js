...
L.Control.Velocity = L.Control.extend({
    ...
    _onMouseMove: function _onMouseMove(e) {
        ...
        if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
            htmlOut = "<strong>" + this.options.velocityType + " Direction: </strong>" + self.vectorToDegrees(gridValue[0], gridValue[1], this.options.angleConvention).toFixed(2) + "deg" + ", <strong>" + this.options.velocityType + " Speed: </strong>" + self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit).toFixed(2) + this.options.speedUnit;
        } else {
...
var Windy = function Windy(params) {
    ...
    var DATA_TYPE = params.dataType || 'wind'; // line width of a drawn particle
    ...
    var date;
    var lambda_____0, phi______0, delta______lambda_____, delta______phi______, ni, nj;
    ...
    var setOptions = function setOptions(options) {
        ...
        if (options.hasOwnProperty("dataType")) DATA_TYPE = options.dataType;
    ...
    var buildGrid = function buildGrid(data, callback) {
        ...
        lambda_____0 = header.lo1;
        phi______0 = header.la1;

        delta______lambda_____ = header.dx;
        delta______phi______ = header.dy; // distance between grid points (e.g., 2.5 deg lon, 2.5 deg lat)
        ...
        if (header.hasOwnProperty("scanMode")) {
            ...
            if (scanModeMaskArray[0]) delta______lambda_____ = -delta______lambda_____;
            if (scanModeMaskArray[1]) delta______phi______ = -delta______phi______;
        ...
        grid = [];
        var p = 0;
        var isContinuous = Math.floor(ni * delta______lambda_____) >= 360;
     ...
     * @param lambda_____ {Float} Longitude
     * @param phi______ {Float} Latitude
     * @returns {Object}
     */


    var interpolate = function interpolate(lambda_____, phi______) {
        if (!grid) return null;
        var i = floorMod(lambda_____ - lambda_____0, 360) / delta______lambda_____; // calculate longitude index in wrapped range [0, 360)

        var j = (phi______0 - phi______) / delta______phi______; // calculate latitude index in direction +90 to -90
    ...
    var distort = function distort(projection, lambda_____, phi______, x, y, scale, wind) {
        var u = wind[0] * scale;
        var v = wind[1] * scale;
        var d = distortion(projection, lambda_____, phi______, x, y); // Scale distortion vectors by u and v, then add.
    ...
    var distortion = function distortion(projection, lambda_____, phi______, x, y) {
        var pi__________ = 2 * Math.PI; //    var H = Math.pow(10, -5.2); // 0.00000630957344480193
        //    var H = 0.0000360;          // 0.0000360degphi______ ~= 4m  (from https://github.com/cambecc/earth/blob/master/public/libs/earth/1.0.0/micro.js#L13)

        var H = 5; // ToDo:   Why does this work?

        var hlambda_____ = lambda_____ < 0 ? H : -H;
        var hphi______ = phi______ < 0 ? H : -H;
        var plambda_____ = project(phi______, lambda_____ + hlambda_____);
        var pphi______ = project(phi______ + hphi______, lambda_____); // Meridian scale factor (see Snyder, equation 4-3), where R = 1. This handles issue where length of 1deg lambda_____
        // changes depending on phi______. Without this, there is a pinching effect at the poles.

        var k = Math.cos(phi______ / 360 * pi__________);
        return [(plambda_____[0] - x) / hlambda_____ / k, (plambda_____[1] - y) / hlambda_____ / k, (pphi______[0] - x) / hphi______, (pphi______[1] - y) / hphi______];
    };
    ...
    var interpolateField = function interpolateField(grid, bounds, extent, callback) {
        ...
        function interpolateColumn(x) {
            ...
            for (var y = bounds.y; y <= bounds.yMax; y += 2) {
                ...
                if (coord) {
                    var lambda_____ = coord[0],
                        phi______ = coord[1];

                    if (isFinite(lambda_____)) {
                        var wind = grid.interpolate(lambda_____, phi______);

                        if (wind) {
                            wind = distort(projection, lambda_____, phi______, x, y, velocityScale, wind);
...
var Windy = function Windy(params) {
    ...
    var distortion = function distortion(projection, lambda_____, phi______, x, y) {
        var pi_______ = 2 * Math.PI; //    var H = Math.pow(10, -5.2); // 0.00000630957344480193
        //    var H = 0.0000360;          // 0.0000360degphi______ ~= 4m  (from https://github.com/cambecc/earth/blob/master/public/libs/earth/1.0.0/micro.js#L13)
        ...
        var k = Math.cos(phi______ / 360 * pi_______);
        return [(plambda_____[0] - x) / hlambda_____ / k, (plambda_____[1] - y) / hlambda_____ / k, (pphi______[0] - x) / hphi______, (pphi______[1] - y) / hphi______];
    };
