...
L.Draw.Event.EDITCLICK = 'draw:editclick';
...
L.Edit.SimpleShape = L.Handler.extend({
	...
	_click: function(e) {
		this._map.fire(L.Draw.Event.EDITCLICK, {layer: e.target});
	}
});
...
L.Edit.Rectangle = L.Edit.SimpleShape.extend({
	...
	_click: function(e) {
		this._map.fire(L.Draw.Event.EDITCLICK, {marker: e.target, layer: this});
	}
});
		...
		readableArea: function (area, isMetric, precision) {
			...
			if (isMetric) {
				...
				if (area >= 1000000 && units.indexOf('km') !== -1) {
					areaStr = L.GeometryUtil.formattedNumber(area * 0.000001, precision['km']) + ' km2';
				} else if (area >= 10000 && units.indexOf('ha') !== -1) {
					areaStr = L.GeometryUtil.formattedNumber(area * 0.0001, precision['ha']) + ' ha';
				} else {
					areaStr = L.GeometryUtil.formattedNumber(area, precision['m']) + ' m2';
				}
			} else {
				area /= 0.836127; // Square yards in 1 meter

				if (area >= 3097600) { //3097600 square yards in 1 square mile
					areaStr = L.GeometryUtil.formattedNumber(area / 3097600, precision['mi']) + ' mi2';
				} else if (area >= 4840) { //4840 square yards in 1 acre
					areaStr = L.GeometryUtil.formattedNumber(area / 4840, precision['ac']) + ' acres';
				} else {
					areaStr = L.GeometryUtil.formattedNumber(area, precision['yd']) + ' yd2';
				}
			}
...
L.Toolbar = L.Class.extend({
	...
	_createActions: function (handler) {
		...
		for (var i = 0; i < l; i++) {
		    if(buttons[i].title === "Cancel drawing") continue;
