...
function updateLine(points){
    ...
    routeDecorators = L.polylineDecorator(points, {
        patterns: [
            { repeat: '40', symbol: L.Symbol.marker({rotate: true, markerOptions: {
                icon: L.icon({
...
function onMarkerClick(e) {
    ...
    if(!hopFlag)
        e.target.bindPopup(htmlStr + "<p>"+textStr+"</p><button class='custom-btn-primary' onclick='Android.addCatch(\""+tripPointId+"\")'>"+uiWords['add_catch'].name+"</button>")
    else
